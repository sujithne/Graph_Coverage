    ...
    if (selectedOption === "edgePairCoverage") {
      ...
      for (let i = 0; i < fpath.length; i++) {
        if (temp.includes(i)) {
          ...
          tempRes.push(fpath[i]);
        }
      }
      let str = tempRes.join(" ");
      setTP(str);
      let edgePPairs = edgePairs(graph);
      let str1 = edgePPairs.join(" ");
      setTR(str1);
    } else if (selectedOption === "edgeCoverage") {
      let temp1 = pathEdgeCoverage(fpath2); // test paths for edge coverage, remove the indices from fpath2
      let tempRes = [];
      for (let i = 0; i < fpath2.length; i++) {
        if (temp1.includes(i)) {
        } else {
          tempRes.push(fpath2[i]);
        }
      }
      let str = tempRes.join(" ");
      setTP(str);
      let edges1 = edgesF(graph);
      let str1 = edges1.join(" ");
      setTR(str1);
    } else {
      let temp2 = pathNodeCoverage(fpath3);
      let tempRes = [];
      for (let i = 0; i < fpath3.length; i++) {
        if (temp2.includes(i)) {
        } else {
          tempRes.push(fpath3[i]);
        }
      }
      let str = tempRes.join(" ");
      setTP(str);
      let nodes1 = nodes(graph);
      let str1 = nodes1.join(" ");
      setTR(str1);
    }
     paths = [];
     path = [];
      ...
      <div className="grey-background">
        ...
        <div className="container">
          <div className="text-area-container">
            ...
            <textarea
              className="text-area"
              placeholder="Enter test requirements here..."
              value={tR}
          ...
          <div className="text-area-container">
            ...
            <textarea
              className="text-area"
              placeholder="Enter text fields here..."
              value={tP}
